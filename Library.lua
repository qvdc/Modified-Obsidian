...
function Funcs:AddIcontukiParagraph(config)
	...
	function Funcs:AddPlayersDropdown(Idx, Info)
		...
		local Dropdown = {
			...
			Text = Info.Text,
			Value = Info.Multi and {} or nil,
			Values = GetPlayers(Info.ExcludeLocalPlayer),
		...
		local function GetPlayerImage(Player)
			if not Player or not Dropdown.EnablePlayerImages then
				return nil
			end
			return {
				Url = string.format("rbxthumb://type=AvatarHeadShot&id=%s&w=48&h=48", tostring(Player.UserId)),
			}
		end
		...
		local function GetValueFromRaw(Raw)
		...
		function Dropdown:Display()
			...
			if Info.Multi then
				for Key, RawValue in Dropdown.Values do
					local Value = IsDictionary and Key or RawValue
					local Player = GetValueFromRaw(Value)

					if Dropdown.Value[Value] then
						if not ValueImage then
							ValueImage = GetPlayerImage(Player)
						end
						Str = Str .. GetDisplayText(Player) .. ", "
					end
				end
				Str = Str:sub(1, #Str - 2)
			else
				...
				local Player = GetValueFromRaw(DisplayValue)
				ValueImage = GetPlayerImage(Player)
				Str = DisplayValue and GetDisplayText(Player) or ""
			end
		...
		local function RefreshPlayerList()
			local PlayerList = GetPlayers(Dropdown.ExcludeLocalPlayer)
			Dropdown.Values = PlayerList
		...
		table.insert(Dropdown.Connections, Players.PlayerAdded:Connect(RefreshPlayerList))
		table.insert(Dropdown.Connections, Players.PlayerRemoving:Connect(RefreshPlayerList))
		...
		local function RecomputeFilteredEntries()
			...
			for Key, RawValue in Values do
				local Value = IsDictionary and Key or RawValue
				local Player = GetValueFromRaw(Value)
				local FormattedValue = GetDisplayText(Player)
				...
				local Entry = {
					...
					Player = Player,
					ValueImage = GetPlayerImage(Player),
					SortKey = Player and Player.DisplayName:lower() or "",
					MatchScore = MatchScore,
					Order = #Pending + 1,
				}
		...
		local function CreatePoolRow()
			...
			function Row:UpdateButton()
				...
				if Entry.ValueImage then
					Image.ImageTransparency = Selected and 0 or 0.5
				end
			end
		...
		function Dropdown:RefreshPlayerList()
			RefreshPlayerList()
		end
		...
		local Defaults = (function()
			...
			elseif DefaultType == "string" then
				...
				for _, P in Dropdown.Values do
					if P and P.Name:lower() == Default:lower() then
			...
			elseif DefaultType == "table" then
				...
				local Value = ResolveOne(Default)
				if Value ~= nil then
					table.insert(Resolved, IsDictionary and Default or Value)
				end
			end
	...
	function Funcs:AddDropdownEx(Idx, Info)
		...
		local Dropdown = {
			...
			MaxVisibleItems = Info.MaxVisibleItems,
		}

		-- Setup default values
		if typeof(Info.Default) == "string" and Info.Default ~= "" then
		...
		MenuTable = Library:AddContextMenu(
			...
			2,
			function(Active: boolean)
				DisplayButton.TextTransparency = Active and 1 or 0
		...
		local ItemHeight = Info.ElementHeight
		local PoolSize = math.max(1, Info.MaxVisibleItems + 2)
		local Pool = {}
		local FilteredEntries = {}

		function Dropdown:RecalculateListSize(Count)
			local ItemCount = Count or #FilteredEntries
			local Y = math.clamp(ItemCount * ItemHeight, 0, Info.MaxVisibleItems * ItemHeight)
		...
		function Dropdown:UpdateColors()
			...
			DisplayImage.ImageTransparency = Dropdown.Disabled and 0.8 or 0
			ArrowImage.ImageTransparency = Dropdown.Disabled and 0.8 or MenuTable.Active and 0 or 0.5
			DisplayContainer.BackgroundTransparency = Dropdown.Disabled and 0.8 or Info.Transparency
		end
		...
		function Dropdown:Display()
			...
			local SelectedNames = {}
			for Name, Selected in Dropdown.Value do
				if Selected then
			...
			if #SelectedNames > 0 then
				DisplayButton.Text = table.concat(SelectedNames, ", ")
			else
				DisplayButton.Text = "unknown"
			end
		end
		...
		function Dropdown:RunChanged()
			...
			local SelectedList = {}
			for Name, _ in Dropdown.Value do
			...
			if Dropdown.Multi then
				Library:SafeCallback(Dropdown.Callback, SelectedList)
				Library:SafeCallback(Dropdown.Changed, SelectedList)
			else
				local SingleValue = SelectedList[1] or ""
				Library:SafeCallback(Dropdown.Callback, SingleValue)
				Library:SafeCallback(Dropdown.Changed, SingleValue)
			end
		end
		...
		local function RecomputeFilteredEntries()
			...
			for _, Value in ipairs(Values) do
				...
				local OptionName = IsTable and Value.Name or Value
		...
		function Dropdown:RefreshPool()
			...
			for SlotIndex, Row in Pool do
				...
				Row.Button.Text = Entry.DisplayText

				if Entry.Icon then
					Row.Image.Visible = true
					local IconData = Library:GetCustomIcon(Entry.Icon)
					if IconData then
		...
		local function CreatePoolRow()
			...
			local Image = New("ImageLabel", {
				...
				ImageColor3 = "FontColor",
				Size = UDim2.fromOffset(16, 16),
				Position = UDim2.fromOffset(4, math.floor((ItemHeight - 16) / 2)),
				Visible = false,
				Parent = Container,
			})
			...
			function Row:UpdateButton()
				...
				local Selected = Dropdown.Value[Entry.Value] == true
			...
			table.insert(Dropdown.Connections, Button.MouseButton1Click:Connect(function()
				...
				if Dropdown.Multi then
					Dropdown.Value[Entry.Value] = not Dropdown.Value[Entry.Value]
				else
